...
contract LiquidationsHelperTest is BaseTest, IUniswapV3SwapCallback {
    ...
    function test_leverage_liquidation() public {
        ...
        uniswapV3Testing.positionManager.safeTransferFrom(ALICE, address(uniswapV3Wrapper), tokenId, "");

        IPool pool = IPool(poolTesting.addressesProvider.getPool());

        pool.supplyERC1155(address(uniswapV3Wrapper), tokenId, uniswapV3Wrapper.balanceOf(ALICE, tokenId), ALICE, 0);
        pool.borrow(address(usdc), 1000e6, 0, ALICE);
        ...
        users[1] = BOB;
        users[2] = ADMIN;
        users = helper.getLiquidatableOnly(users);
        require(users.length == 3);
        require(users[0] == ALICE);
        require(users[1] == address(0));
        require(users[2] == address(0));
        (LiquidationsHelper.UserCollateralInfo memory collateral, LiquidationsHelper.UserDebtInfo memory debt) =
            helper.getBestLiquidationOption(ALICE);

        assertEq(collateral.collateralAsset, address(uniswapV3Wrapper));
        assertEq(collateral.collateralTokenId, tokenId);
        assertEq(collateral.erc1155, true);
        assertEq(debt.debtAsset, address(usdc));
